...
impl Clipboard {
    ...
    pub fn get_text(&mut self) -> Result<String> {
        ...
        match self.inner.get_text() {
            Ok(text) => Ok(text),
            // Empty or non-text clipboard should not break command execution.
            Err(_) => Ok(String::new()),
        }
    }
}
    ...
    fn get_text_linux(&mut self) -> Result<String> {
        for backend in available_read_backends() {
            ...
            if let Ok(text) = result {
                // Some providers report success but return empty payload for certain copy sources.
                // In that case, keep trying other backends before giving up.
...
fn available_read_backends() -> Vec<LinuxClipboardBackend> {
    ...
    // Some web "Copy" buttons may end up in selection buffers; read them as a fallback.
    if is_wayland && command_exists("wl-paste") {
